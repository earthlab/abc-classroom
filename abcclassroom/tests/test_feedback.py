...
def create_directories(default_config, tmp_path):
    ...
    materials_dir = default_config["materials_dir"]
...
def test_copy_feedback_files(default_config, tmp_path):
    "Tests that copying the feedback files copies both files"

    default_config["course_directory"] = tmp_path
    abcfeedback.copy_feedback_files(
        default_config["assignment"],
        default_config["roster_filename"],
        default_config["course_dir"],
        default_config["materials_dir"],
        default_config["clone_dir"],
    )
